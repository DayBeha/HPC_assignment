...
for STREAM_ARRAY_SIZE in Size_list:
    ...
    print(f"Copy\t time use:{times[0]*10e6:.2f} us\t data amount:{copy}\t bandwith:{copy/times[0]/10e6:.3f} MB/s ")
    print(f"add\t\t time use:{times[1]*10e6:.2f} us\t data amount:{add}\t bandwith:{add/times[1]/10e6:.3f} MB/s")
    print(f"scale\t time use:{times[2]*10e6:.2f} us\t data amount:{scale}\t bandwith:{scale/times[2]/10e6:.3f} MB/s")
    print(f"triad\t time use:{times[3]*10e6:.2f} us\t data amount:{triad}\t bandwith:{triad/times[3]/10e6:.3f} MB/s\n")
